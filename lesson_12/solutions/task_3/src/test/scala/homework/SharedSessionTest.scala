...
import homework.DataApiHomeWorkTaxi.{init, readInfo, readStats}
...
class SharedSessionTest extends SharedSparkSession {
  ...
  test("Most Popular") {
    ...
    val taxiInfoDF  = readInfo(taxiInfoFile)

    val mostPopularDF = MostPopularDF.process(taxiFactsDF, taxiInfoDF)
  ...
  test("Order Distribution") {
    ...
    val dist = OrderDistributionDS.process(taxiFactsDF)
